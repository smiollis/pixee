...
echo "==================================="
echo "Installation des environnements Docker pour macOS"
echo "==================================="
echo ""
...
if ! command -v docker &> /dev/null; then
    echo "Docker n'est pas installé. Veuillez installer Docker Desktop pour Mac avant de continuer."
...
if ! command -v docker-compose &> /dev/null; then
    echo "Docker Compose n'est pas installé. Il est normalement inclus avec Docker Desktop pour Mac."
    echo "Veuillez vérifier votre installation Docker."
    exit 1
fi

# Vérifier si Docker est en cours d'exécution
if ! docker info &> /dev/null; then
    echo "Docker n'est pas en cours d'exécution. Veuillez démarrer Docker Desktop avant de continuer."
    exit 1
fi
...
echo "Note: Sur macOS, vous devrez peut-être autoriser Docker à accéder aux dossiers montés."
echo "Si vous rencontrez des problèmes de permissions, allez dans Préférences Docker > Resources > File Sharing"
echo "et ajoutez les dossiers nécessaires."
